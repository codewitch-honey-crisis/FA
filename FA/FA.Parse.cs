					...
					case '\\':
						...
						switch (pc.Current)
						{
							...
							case 'p':
								...
								if(isNot)
								{
									next = FA.Set(CharCls.UnicodeCategories[uci], accept);
								} else
									next = FA.Set(CharCls.NotUnicodeCategories[uci], accept);
								break;
							case 'd':
								next = FA.Set(CharCls.digit, accept);
								pc.Advance();
								break;
							case 'D':
								next = FA.Set(RangeUtility.NotRanges(CharCls.digit), accept);
								pc.Advance();
								break;

							case 's':
								next = FA.Set(CharCls.space,accept);
								pc.Advance();
								break;
							case 'S':
								next = FA.Set(RangeUtility.NotRanges(CharCls.space),accept);
								pc.Advance();
								break;
							case 'w':
								next = FA.Set(CharCls.word,accept);
								pc.Advance();
								break;
							case 'W':
								next = FA.Set(RangeUtility.NotRanges(CharCls.word),accept);
								pc.Advance();
								break;
							default:
				...
				if (!wantRange)
				{
					...
					if ('[' == pc.Current) // named char class
					{
						...
						else
						{
							...
							int[] ranges;
							if (!CharCls.CharacterClasses.TryGetValue(cls, out ranges))
