...
{var reg=new List<int[]>();Compiler.EmitAstInnerPart(inst.Expr,reg);regm.Add(inst,reg.ToArray());pc+=reg.Count;}else++pc;}pc=0;for(var i=0;i<ic;++i){int
 dst;var inst=instructions[i];int[]code=null;switch(inst.Opcode){case Inst.Regex:Compiler.Fixup(regm[inst],pc);result.AddRange(regm[inst]);break;case Inst.Label:
...
public int Kind=None;public bool IsLazy=false;public Ast[]Exprs=null;public int Value='\0';public int[]Ranges;public int Min=0;public int Max=0;static
 ...
 ToFA(int match){Ast ast=this;if(ast.IsLazy)throw new NotSupportedException("The AST node cannot be lazy");switch(ast.Kind){case Ast.Alt:return FA.Or(_ToFAs(ast.Exprs,
...
int>>(RangeUtility.NotRanges(pairs));return FA.Set(RangeUtility.FromPairs(pairl),match);case Ast.Opt:return FA.Optional(ast.Exprs[0].ToFA(match),match);
case Ast.Plus:return FA.Repeat(ast.Exprs[0].ToFA(match),1,0,match);case Ast.Rep:return FA.Repeat(ast.Exprs[0].ToFA(match),ast.Min,ast.Max,match);case Ast.Set:
return FA.Set(ast.Ranges,match);case Ast.Star:return FA.Repeat(ast.Exprs[0].ToFA(match),0,0,match);case Ast.UCode:return FA.Set(CharacterClasses.UnicodeCategories[ast.Value],
match);default:throw new NotImplementedException();}}public static Ast FromLiteral(IEnumerable<char>literal){var exprs=new List<Ast>();foreach(var ich
 in UnicodeUtility.ToUtf32(literal)){var ast=new Ast();ast.Kind=Lit;ast.Value=ich;exprs.Add(ast);}if(0==exprs.Count)throw new ArgumentException("The string cannot be empty",
"literal");if(1==exprs.Count)return exprs[0];var result=new Ast();result.Kind=Cat;result.Exprs=exprs.ToArray();return result;}public static Ast Parse(LexContext
 pc){Ast result=null,next=null;int ich;pc.EnsureStarted();while(true){switch(pc.Current){case-1:return result;case'.':var dot=new Ast();dot.Kind=Ast.Dot;
if(null==result)result=dot;else{if(Ast.Cat==result.Kind){var exprs=new Ast[result.Exprs.Length+1];Array.Copy(result.Exprs,0,exprs,0,result.Exprs.Length);
exprs[exprs.Length-1]=dot;result.Exprs=exprs;}else{var cat=new Ast();cat.Kind=Ast.Cat;cat.Exprs=new Ast[]{result,dot};result=cat;}}pc.Advance();result
=_ParseModifier(result,pc);break;case'\\':pc.Advance();pc.Expecting();var isNot=false;switch(pc.Current){case'P':isNot=true;goto case'p';case'p':pc.Advance();
pc.Expecting('{');var uc=new StringBuilder();int uli=pc.Line;int uco=pc.Column;long upo=pc.Position;while(-1!=pc.Advance()&&'}'!=pc.Current)uc.Append((char)pc.Current);
...
case"Zs":uci=11;break;case"Mc":uci=6;break;case"Cs":uci=16;break;case"Lt":uci=2;break;case"Lu":uci=0;break;}next=new Ast();next.Value=uci;next.Kind=isNot?Ast.NUCode:Ast.UCode;
break;case'd':next=new Ast();next.Kind=Ast.Set;next.Ranges=new int[]{'0','9'};pc.Advance();break;case'D':next=new Ast();next.Kind=Ast.NSet;next.Ranges
=new int[]{'0','9'};pc.Advance();break;case's':next=new Ast();next.Kind=Ast.Set;next.Ranges=new int[]{'\t','\t',' ',' ','\r','\r','\n','\n','\f','\f'};
pc.Advance();break;case'S':next=new Ast();next.Kind=Ast.NSet;next.Ranges=new int[]{'\t','\t',' ',' ','\r','\r','\n','\n','\f','\f'};pc.Advance();break;
case'w':next=new Ast();next.Kind=Ast.Set;next.Ranges=new int[]{'_','_','0','9','A','Z','a','z',};pc.Advance();break;case'W':next=new Ast();next.Kind=Ast.NSet;
next.Ranges=new int[]{'_','_','0','9','A','Z','a','z',};pc.Advance();break;default:if(-1!=(ich=_ParseEscapePart(pc))){next=new Ast();next.Kind=Ast.Lit;
next.Value=ich;}else{pc.Expecting(); return null;}break;}next=_ParseModifier(next,pc);if(null!=result){if(Ast.Cat==result.Kind){var exprs=new Ast[result.Exprs.Length
+1];Array.Copy(result.Exprs,0,exprs,0,result.Exprs.Length);exprs[exprs.Length-1]=next;result.Exprs=exprs;}else{var cat=new Ast();cat.Kind=Ast.Cat;cat.Exprs
=new Ast[]{result,next};result=cat;}}else result=next;break;case')':return result;case'(':pc.Advance();pc.Expecting();next=Parse(pc);pc.Expecting(')');
pc.Advance();next=_ParseModifier(next,pc);if(null==result)result=next;else{if(Ast.Cat==result.Kind){var exprs=new Ast[result.Exprs.Length+1];Array.Copy(result.Exprs,
...
result=cat;}}break;case'|':if(-1!=pc.Advance()){next=Parse(pc);if(null!=result&&Ast.Lit==result.Kind&&Ast.Lit==next.Kind){var set=new Ast();set.Kind=Set;
set.Ranges=new int[]{result.Value,result.Value,next.Value,next.Value};result=set;}else if(null!=result&&Ast.Lit==result.Kind&&Ast.Set==next.Kind){var set
=new Ast();set.Kind=Ast.Set;set.Ranges=new int[next.Ranges.Length+2];set.Ranges[0]=result.Value;set.Ranges[1]=result.Value;Array.Copy(next.Ranges,0,set.Ranges,
2,next.Ranges.Length);result=set;}else if(null!=result&&Ast.Alt==result.Kind){var exprs=new Ast[result.Exprs.Length+1];Array.Copy(result.Exprs,0,exprs,
0,result.Exprs.Length);exprs[exprs.Length-1]=next;result.Exprs=exprs;}else{var alt=new Ast();alt.Kind=Ast.Alt;if(null==next||next.Kind!=Alt){alt.Exprs
...
next.Kind=(seti.Key)?NSet:Set;next.Ranges=seti.Value;next=_ParseModifier(next,pc);if(null==result)result=next;else{if(Ast.Cat==result.Kind){var exprs=
new Ast[result.Exprs.Length+1];Array.Copy(result.Exprs,0,exprs,0,result.Exprs.Length);exprs[exprs.Length-1]=next;result.Exprs=exprs;}else{var cat=new Ast();
cat.Kind=Ast.Cat;cat.Exprs=new Ast[]{result,next};result=cat;}}break;default:ich=pc.Current;if(char.IsHighSurrogate((char)ich)){if(-1==pc.Advance())throw
 new ExpectingException("Expecting low surrogate in Unicode stream",pc.Line,pc.Column,pc.Position,pc.FileOrUrl,"low-surrogate");ich=char.ConvertToUtf32((char)ich,
(char)pc.Current);}next=new Ast();next.Kind=Ast.Lit;next.Value=ich;pc.Advance();next=_ParseModifier(next,pc);if(null==result)result=next;else{if(Ast.Cat
==result.Kind){var exprs=new Ast[result.Exprs.Length+1];Array.Copy(result.Exprs,0,exprs,0,result.Exprs.Length);exprs[exprs.Length-1]=next;result.Exprs
=exprs;}else{var cat=new Ast();cat.Kind=Ast.Cat;cat.Exprs=new Ast[]{result,next};result=cat;}}break;}}}static KeyValuePair<bool,int[]>_ParseSet(LexContext
 pc){var result=new List<int>();pc.EnsureStarted();pc.Expecting('[');pc.Advance();pc.Expecting();var isNot=false;if('^'==pc.Current){isNot=true;pc.Advance();
...
wantRange=false;firstRead=false;continue;}}if(!readFirstChar){if(char.IsHighSurrogate((char)pc.Current)){var chh=(char)pc.Current;pc.Advance();pc.Expecting();
firstChar=char.ConvertToUtf32(chh,(char)pc.Current);pc.Advance();pc.Expecting();}else if('\\'==pc.Current){pc.Advance();firstChar=_ParseRangeEscapePart(pc);
}else{firstChar=pc.Current;pc.Advance();pc.Expecting();}readFirstChar=true;}else{if('-'==pc.Current){pc.Advance();pc.Expecting();wantRange=true;}else{
result.Add(firstChar);result.Add(firstChar);readFirstChar=false;}}firstRead=false;}else{if('\\'!=pc.Current){var ch=0;if(char.IsHighSurrogate((char)pc.Current))
...
#endregion
internal static List<int[]>Emit(Ast ast,int symbolId=-1){var prog=new List<int[]>();EmitAstInnerPart(ast,prog);if(-1!=symbolId){var match=new int[2];match[0]
...
nameof(literal));ch=char.ConvertToUtf32(literal,i);++i;}var lit=new int[2];lit[0]=Char;lit[1]=ch;prog.Add(lit);}}internal static void EmitAstPart(Ast ast,IList<int[]>
prog,int symbolId=0){EmitAstInnerPart(ast,prog);var save=new int[]{Save,1};prog.Add(save);var match=new int[2];match[0]=Match;match[1]=symbolId;prog.Add(match);
}internal static void EmitAstInnerPart(Ast ast,IList<int[]>prog){int[]inst,jmp;switch(ast.Kind){case Ast.Lit: inst=new int[2];inst[0]=Char;inst[1]=ast.Value;
prog.Add(inst);break;case Ast.Cat: for(var i=0;i<ast.Exprs.Length;i++)if(null!=ast.Exprs[i])EmitAstInnerPart(ast.Exprs[i],prog);break;case Ast.Dot: inst
=new int[1];inst[0]=Any;prog.Add(inst);break;case Ast.Alt: var exprs=new List<Ast>(ast.Exprs.Length);var firstNull=-1;for(var i=0;i<ast.Exprs.Length;i++)
{var e=ast.Exprs[i];if(null==e){if(0>firstNull){firstNull=i;exprs.Add(null);}continue;}exprs.Add(e);}ast.Exprs=exprs.ToArray();var jjmp=new int[ast.Exprs.Length
+1];jjmp[0]=Jmp;prog.Add(jjmp);var jmpfixes=new List<int>(ast.Exprs.Length-1);for(var i=0;i<ast.Exprs.Length;++i){var e=ast.Exprs[i];if(null!=e){jjmp[i
+1]=prog.Count;EmitAstInnerPart(e,prog);if(i==ast.Exprs.Length-1)continue;if(i==ast.Exprs.Length-2&&null==ast.Exprs[i+1])continue;var j=new int[2];j[0]
=Jmp;jmpfixes.Add(prog.Count);prog.Add(j);}}for(int ic=jmpfixes.Count,i=0;i<ic;++i){var j=prog[jmpfixes[i]];j[1]=prog.Count;}if(-1<firstNull){jjmp[firstNull
+1]=prog.Count;}break;case Ast.NSet:case Ast.Set: inst=new int[ast.Ranges.Length+1];inst[0]=(ast.Kind==Ast.Set)?Set:NSet;SortRanges(ast.Ranges);Array.Copy(ast.Ranges,
0,inst,1,ast.Ranges.Length);prog.Add(inst);break;case Ast.NUCode:case Ast.UCode: inst=new int[2];inst[0]=(ast.Kind==Ast.UCode)?UCode:NUCode;inst[1]=ast.Value;
prog.Add(inst);break;case Ast.Opt:inst=new int[3]; inst[0]=Jmp;prog.Add(inst);inst[1]=prog.Count; for(var i=0;i<ast.Exprs.Length;i++)if(null!=ast.Exprs[i])
EmitAstInnerPart(ast.Exprs[i],prog);inst[2]=prog.Count;if(ast.IsLazy){ var t=inst[1];inst[1]=inst[2];inst[2]=t;}break; case Ast.Star:ast.Min=0;ast.Max
=0;goto case Ast.Rep;case Ast.Plus:ast.Min=1;ast.Max=0;goto case Ast.Rep;case Ast.Rep: if(ast.Min>0&&ast.Max>0&&ast.Min>ast.Max)throw new ArgumentOutOfRangeException("Max");
int idx2;Ast opt;Ast rep;switch(ast.Min){case-1:case 0:switch(ast.Max){case-1: case 0:var idx=prog.Count;inst=new int[3];inst[0]=Jmp;prog.Add(inst);idx2
=prog.Count;for(var i=0;i<ast.Exprs.Length;i++)if(null!=ast.Exprs[i])EmitAstInnerPart(ast.Exprs[i],prog);inst[1]=idx2;jmp=new int[2];jmp[0]=Jmp;jmp[1]
=idx;prog.Add(jmp);inst[2]=prog.Count;if(ast.IsLazy){ var t=inst[1];inst[1]=inst[2];inst[2]=t;}return;case 1: opt=new Ast();opt.Kind=Ast.Opt;opt.Exprs
=ast.Exprs;opt.IsLazy=ast.IsLazy;EmitAstInnerPart(opt,prog);return;default: opt=new Ast();opt.Kind=Ast.Opt;opt.Exprs=ast.Exprs;opt.IsLazy=ast.IsLazy;EmitAstInnerPart(opt,
prog);for(var i=1;i<ast.Max;++i){EmitAstInnerPart(opt,prog);}return;}case 1:switch(ast.Max){ case-1:case 0:idx2=prog.Count;for(var i=0;i<ast.Exprs.Length;
i++)if(null!=ast.Exprs[i])EmitAstInnerPart(ast.Exprs[i],prog);inst=new int[3];inst[0]=Jmp;prog.Add(inst);inst[1]=idx2;inst[2]=prog.Count;if(ast.IsLazy)
{ var t=inst[1];inst[1]=inst[2];inst[2]=t;}return;case 1: for(var i=0;i<ast.Exprs.Length;i++)if(null!=ast.Exprs[i])EmitAstInnerPart(ast.Exprs[i],prog);
return;default: rep=new Ast();rep.Min=0;rep.Max=ast.Max-1;rep.IsLazy=ast.IsLazy;rep.Exprs=ast.Exprs;for(var i=0;i<ast.Exprs.Length;i++)if(null!=ast.Exprs[i])
EmitAstInnerPart(ast.Exprs[i],prog);EmitAstInnerPart(rep,prog);return;}default: switch(ast.Max){ case-1:case 0:for(var j=0;j<ast.Min;++j){for(var i=0;
i<ast.Exprs.Length;i++)if(null!=ast.Exprs[i])EmitAstInnerPart(ast.Exprs[i],prog);}rep=new Ast();rep.Kind=Ast.Star;rep.Exprs=ast.Exprs;rep.IsLazy=ast.IsLazy;
EmitAstInnerPart(rep,prog);return;case 1: throw new NotImplementedException();default: for(var j=0;j<ast.Min;++j){for(var i=0;i<ast.Exprs.Length;i++)if
(null!=ast.Exprs[i])EmitAstInnerPart(ast.Exprs[i],prog);}if(ast.Min==ast.Max)return;opt=new Ast();opt.Kind=Ast.Opt;opt.Exprs=ast.Exprs;opt.IsLazy=ast.IsLazy;
rep=new Ast();rep.Kind=Ast.Rep;rep.Min=rep.Max=ast.Max-ast.Min;EmitAstInnerPart(rep,prog);return;}} throw new NotImplementedException();}}internal static
...
{try{fa=ast.ToFA(0);} catch(NotSupportedException){}if(null!=fa){Compiler.EmitFAPart(fa,prog);prog=new List<int[]>(Compiler.RemoveDeadCode(prog));return
 prog.ToArray();}}Compiler.EmitAstInnerPart(ast,prog);return prog.ToArray();}/// <summary>
/// Compiles a single regular expression into a program segment
/// </summary>
...
{_EnqueueFiber(ref nextFiberCount,ref nextFibers,new _Fiber(t,pc[idx],saved),sp);++idx;}}break;case Compiler.Char:if(cur!=pc[1]){break;}goto case Compiler.Any;
case Compiler.Set:idx=1;if(!_InRanges(pc,ref idx,cur)){break;}goto case Compiler.Any;case Compiler.NSet:idx=1;if(_InRanges(pc,ref idx,cur)){break;}goto
 case Compiler.Any;case Compiler.UCode:var str=char.ConvertFromUtf32(cur);if(unchecked((int)char.GetUnicodeCategory(str,0)!=pc[1])){break;}goto case Compiler.Any;
case Compiler.NUCode:str=char.ConvertFromUtf32(cur);if(unchecked((int)char.GetUnicodeCategory(str,0))==pc[1]){break;}goto case Compiler.Any;case Compiler.Any:
if(LexContext.EndOfInput==input.Current){break;}passed=true;_EnqueueFiber(ref nextFiberCount,ref nextFibers,new _Fiber(t,t.Index+1,saved),sp+1);break;
case Compiler.Match:matched=saved;match=pc[1]; i=currentFiberCount;break;}}if(passed){sb.Append(char.ConvertFromUtf32(cur));input.Advance();if(LexContext.EndOfInput
...
=matched[0]; var len=matched[1];input.CaptureBuffer.Append(sb.ToString(start,len-start));return match;};return-1;}/// <summary>
